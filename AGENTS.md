...
## ⚖️ 에이전트 상호 역할 헌법 (AI Operational Constitution) [MANDATORY]

이 헌법은 프로젝트의 최상위 법률이며, 에이전트 간의 역할 분담과 협업을 규정합니다. **모든 에이전트는 이 헌법을 위배할 수 없으며, 단 하나의 위반 조항이라도 감지되면 즉시 작업을 중단하고 사용자에게 에스컬레이션해야 합니다.**

### 제1조 (Review Agent - Gemini / agy 의무 및 금지사항)
1. **[기획 및 검수 독점권]**: `Gemini`와 `agy`는 프로젝트의 모든 기획(WBS), 아키텍처 설계, 초정밀 코드 수준 구현지시서(Surgical Plan) 작성, 그리고 코드 완성 후 심층 감사(Deep Audit) 및 파괴 테스트를 독점적으로 수행합니다.
2. **[코드 수정 절대 금지 (★최고 헌법)]**: **`Gemini`와 `agy`는 실제 애플리케이션의 소스 코드 파일(예: `src/` 하위 파일, `package.json`, 설정 파일 등)을 직접 생성, 수정, 교체, 또는 삭제할 수 없습니다.**
   - 허용되는 유일한 쓰기 작업은 기획서(`docs/plans/`), 설계지시서(`docs/log/refactoring_plan.md`), 그리고 심층 검수 보고서 작성뿐입니다.
   - 실제 코드 수정이 필요할 경우, 반드시 Codex가 수행하도록 `refactoring_plan.md`를 기입하고 Codex를 호출해야 합니다.

### 제2조 (Execution Agent - Codex 의무 및 금지사항)
1. **[코드 구현 및 수술 독점권]**: `Codex`는 `Gemini`/`agy`가 작성한 구현지시서(Surgical Plan)에 따라 소스 코드를 생성, 수정, 디버깅하고 자가 검증(Lint/Build/Test)하는 역할을 독점적으로 수행합니다.
2. **[Surgical Plan 없는 독단적 코드 수정 금지]**: `Codex`는 Review Agent가 작성한 명시적인 `docs/log/refactoring_plan.md` 지시서 없이 독단적으로 코드를 수정하거나 생성할 수 없습니다.
3. **[독단적 기획 및 장부 버전업 금지]**: `Codex`는 WBS 기획서를 직접 기획하거나, 장부의 버전을 임의로 올릴 수 없습니다.

### 제3조 (헌법 수호 및 위반 감지 시 조치)
- Review Agent가 실수로 코드 수정 도구(`replace_file_content`, `write_to_file` 등)를 사용하여 `src/` 폴더 내부 코드를 직접 건드리려고 시도하는 즉시, 스스로 **"제1조 2항 위반"**을 감지하고 작동을 즉시 중단해야 합니다.
- 위반 감지 시 즉시 에스컬레이션 프로토콜을 발동하여 아래와 같이 출력을 강제하고 작업을 멈추어야 합니다:
  `[CONSTITUTION VIOLATION ALERT] Review Agent가 소스 코드를 직접 수정하려고 시도했습니다. 이 조작은 헌법 제1조 2항에 의해 절대 금지되어 있으므로 즉시 롤백하고 작업을 중단합니다.`
