 ...
 *
 * Works with: Claude Code (Stop), Codex CLI (Stop), agy/Gemini CLI (AfterAgent)
 *
...
function detectVendor(input: Record<string, unknown>): Vendor {
  ...
  if (event === "AfterAgent" && process.env.GEMINI_PROJECT_DIR)
    return "gemini";
...
async function main() {
  ...
  const textToCheck = [
    input.prompt_response, // Gemini-family AfterAgent
    input.response,
    input.content,
    input.message,
    input.transcript,
