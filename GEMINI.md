...
Gemini는 `AGENTS.md`에 명시된 **에이전트 상호 역할 헌법(AI Operational Constitution)**을 절대적으로 수호해야 합니다.

1. **[기획, 설계, 검수만 수행]**: Gemini는 오직 기획(`docs/plans/`), 설계지시서(`docs/log/refactoring_plan.md`), 그리고 심층 검수만 작성 및 수행해야 합니다.
2. **[코드 직접 수정 절대 금지 (★최고 존엄)]**: **Gemini는 실제 애플리케이션의 소스 코드 파일(예: `src/` 하위 파일, `package.json` 등)을 직접 수정하거나 생성하는 어떠한 파일 편집 도구(replace_file_content, write_to_file 등)도 직접 사용할 수 없습니다.**
