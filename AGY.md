...
## HIGH PRIORITY: agy → Codex Delegation Gate [TARGET: AGY ONLY - DO NOT APPLY IF YOU ARE CODEX]

> [!IMPORTANT]
> **이 섹션은 오직 [agy] 에이전트에게만 적용됩니다. 당신이 [Codex] 에이전트라면 이 섹션의 모든 규칙을 완전히 무시하고 건너뛰십시오. 절대로 Codex 스스로에게 위임 여부를 묻는 모순적인 루프에 빠져서는 안 됩니다.**

당신이 **agy** 에이전트일 때만 이 규칙을 적용합니다. 이 프로젝트에서는 agy CLI와 Codex CLI를 tmux 2분할 환경에서 함께 사용한다.
사용자의 요청이 아래 유형에 해당하면 agy는 바로 직접 처리하지 말고, 먼저 Codex 위임 여부를 사용자에게 물어봐야 한다.
...
| :-------------------- | :------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------- |
| **agy (전략/검토)**   | `pm-planner`, `qa-reviewer`, `architecture-reviewer`, `debug-investigator` (원인 분석), `docs-curator`                           | 요구사항 분석, 코드 품질 리뷰, 설계 검토, 버그 원인 파악, 문서 정합성 체크 |
...
사용자의 요청에 아래 표현이나 의미가 포함되면, agy는 직접 처리하지 말고 먼저 Codex 위임 여부를 물어본다.
...
- 최종 산출물이 리서치, 비교, 기획, 아이디어 정리, 문서 검토라면 agy가 직접 처리하고 Codex로 넘기지 않는다.
- **이력 누적형 검토 및 수정계획 수립 프로토콜 (Ledger 활용)**:
  - **수정 계획 수립 시**: agy는 계획을 [docs/log/refactoring_plan.md](file:///home/sc/TestProject/docs/log/refactoring_plan.md) 파일에 저장함. 파일이 이미 존재할 경우 덮어쓰지 않고, 파일 하단에 `## [신규 계획 - YYYY-MM-DD HH:MM:SS]` 섹션을 생성하여 **추가(Append)**하며, Codex에게는 오직 **새로 추가된 이 증분 계획 섹션만 읽고** 작업을 개시하도록 지시함.
  - **Codex 수정 완료 및 요약 보고 수신 시**: Codex는 수정을 마친 후 상세 수정 결과를 [docs/logs/modification_log.md](file:///home/sc/TestProject/docs/logs/modification_log.md) 파일 하단에 `## [신규 수정 보고 - YYYY-MM-DD HH:MM:SS]` 섹션으로 **추가(Append)하여 저장**하고 agy에게 알림. agy는 해당 파일의 **가장 하단에 새로 추가된 신규 수정 보고 섹션만 읽어서(Incremental Reading)** 변경 내역을 신속하게 파악함.
  - **철저한 코드레벨 2차 검증 및 피드백**: agy는 단순히 요약문만 믿지 않고, 실제 수정된 해당하는 소스 파일들을 직접 열어(`view_file` 등 활용) 코드를 한 줄씩 면밀히 검토함. 결함이나 개선이 필요할 경우, [docs/log/refactoring_plan.md](file:///home/sc/TestProject/docs/log/refactoring_plan.md) 하단에 `## [신규 피드백/보완 계획 - YYYY-MM-DD HH:MM:SS]` 형태로 추가 기재하여 Codex에게 이 신규 추가 섹션만 읽고 보완하게끔 피드백 루프를 가동함.
...
agy가 리뷰/분석을 완료한 뒤 그 결과를 바탕으로 파일 수정이 필요하다고 판단한 경우, 사용자가 이미 "수정까지 진행", "Codex에 맡겨", "알아서 넘겨", "자동으로 실행"처럼 자동 수정 위임 의사를 밝힌 상태라면 별도의 추가 확인 없이 Codex로 위임한다.

이때 agy는 사용자에게 다시 "Codex CLI에 보내서 실행할까요?"라고 묻지 않는다. 대신 다음 명령을 바로 실행한다.
...
자동 위임은 agy 리뷰 결과를 바탕으로 한 코드/문서/설정 파일 수정에만 허용한다. 민감정보 처리, destructive command, 대규모 삭제, 배포, 결제/권한 변경, 외부 서비스 설정 변경은 자동 위임하지 말고 사용자 승인을 다시 받는다.

### 반드시 먼저 물어볼 문장

위 조건에 해당하면 agy는 작업을 시작하기 전에 반드시 아래 문장을 먼저 출력한다.
...
- **장부 기반 계획 전달**: 수정 계획이 `docs/log/` 내 활성화된 계획 파일(예: `refactoring_plan.md` 또는 `refactoring_plan_2.md`)에 저장된 경우, Codex 프롬프트에는 계획 전문을 붙여넣지 말고 해당 파일 경로와 읽어야 할 최신 섹션 제목만 포함해야 함. Codex에게 해당 파일의 가장 마지막 `## [신규 계획 - ...]` 또는 `## [신규 피드백/보완 계획 - ...]` 섹션만 읽도록 명시해야 함
- **우선순위 기반 순차 수정 계획**: 수정이 필요한 사항들을 한꺼번에 처리하지 않고, 중요도와 종속성에 따라 우선순위를 매긴 명확한 단계별 목록(예: 1단계, 2단계, 3단계)으로 구조화하여 제공해야 함
- **Codex 내부 자가 검증식 일괄 처리 지시 (비용 최적화)**: Codex에게 각 단계별로 매번 대화를 끊고 승인을 받지 말도록 지시함. 대신, 단일 프롬프트 안에서 순차적 우선순위에 따라 수정하되 **각 단계를 반영한 직후 쉘 명령어(`run_command`)를 활용해 테스트(`node test.js` 등)를 스스로 수행하는 내부 자가 검증 루프**를 돌리도록 지시함. 모든 단계의 자가 검증이 무사히 성공했을 때만 단 한 번 최종 소스 코드 및 통합 수정 보고서(`modification_log.md`)를 작성하도록 프로세스를 정의함.
- 수정 허용 범위
- 기존 구조를 최대한 유지하라는 지시
