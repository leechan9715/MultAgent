...
| **agy** | Google agy 계열 .agy-3.1-pro) | 리서치, 기획, 문서 검토, QA 리뷰, 트렌드 분석 |
...
1. **사용자 직접 전송**: 사용자가 `ask-codex.sh`나 `ask-agy.sh`를 사용해 특정 창에 질문을 보냅니다.
2. **상호 위임 규칙**: 만약 질문의 성격이 담당 부서와 맞지 않으면(예: agy 창에 코딩 요청), 각 부서의 지침서(`AGENTS.md`, `AGY.md`)에 따라 AI가 사용자에게 다른 창으로 보낼지 묻습니다.
...
2. **모델 매핑**: `.agents/oma-config.yaml` 설정과 각 부서 지침서(`AGENTS.md`, `AGY.md`)의 스킬 매핑 표를 기준으로 해당 작업에 가장 적합한 모델 및 에이전트를 선택합니다.
   - 예: **Frontend** 구현 질문 -> `gpt-5.5` 모델의 `frontend-engineer` 에이전트 호출
   - 예: **QA** 리뷰 질문 -> .agy-3.1-pro` 모델의 `qa-reviewer` 에이전트 호출
...
- **agy 에이전트**: `.agy/agents/*.md` (agy 기반 전문가 정의)
- **공통 규칙**: `AGENTS.md`, `AGY.md` (부서 간 협업 및 위임 규칙)

이 아키텍처 덕분에 사용자는 매번 모델을 지정할 필요 없이, 평소처럼 질문하는 것만으로도 뒤에서 가장 적합한 전문가 AI가 배정되어 응답하게 됩니다.

이 문서는 `/home/sc/TestProject`에 구성된 Codex + agy 2분할 AI 작업 환경을 처음 받은 사람이 Ubuntu 또는 Windows(WSL)에서 바로 실행하고 이해할 수 있도록 정리한 가이드입니다.
...
이 명령을 실행하면 왼쪽에는 Codex, 오른쪽에는 agy가 자동으로 열립니다. 처음 사용하는 사람은 먼저 이 방식으로 실행하고, 문제가 생겼을 때만 아래의 자세한 설치/수동 실행 설명을 보면 됩니다.
...
일반적으로 사용자는 Codex 쪽을 메인 작업 창으로 쓰고, 리서치/기획/리뷰가 필요할 때 agy로 넘기는 흐름을 사용합니다.

초보자 관점에서는 이렇게 이해하면 됩니다.

```text
Codex 창 = 실제로 코드를 고치고 명령을 실행하는 작업자
agy 창 = 조사하고 검토하고 방향을 정리하는 리뷰어
.codex-pane / .agy-pane = 두 창의 내부 주소를 적어 둔 파일
ask-codex.sh / ask-agy.sh = 그 주소로 메시지를 보내는 스크립트
```

`.codex-pane`, `.agy-pane` 같은 파일은 사용자가 직접 작성하는 프롬프트가 아닙니다. 두 AI 창의 전화번호를 적어 둔 주소록에 가깝습니다. `scripts/start-ai.sh`가 이 번호를 자동으로 찾아 파일에 적어 두기 때문에, 나중에 `ask-codex.sh`나 `ask-agy.sh`가 어느 창으로 메시지를 보내야 하는지 스스로 찾을 수 있습니다.
...
.agy/agents/*.md
- agy native agent 정의
- 분석/리뷰 계열 agent는.agy-3.1-pro-preview 기준
...
scripts/ask-agy.sh
- 현재 shell에서 agy pane으로 프롬프트 전송
```

주의: `.agents/` 디렉터리는 oh-my-agent의 SSOT 영역입니다. 일반 작업 중 직접 수정하지 않습니다. 설정 변경은 보통 `.agents/oma-config.yaml`, `AGENTS.md`, `AGY.md`, 또는 생성된 agent 파일 쪽에서 의도를 확인한 뒤 진행합니다.

커스텀 규칙을 `AGENTS.md`나 `AGY.md`에 추가해야 한다면 반드시 `<!-- OMA:END -->` 태그 아래에 둡니다. `<!-- OMA:START -->`부터 `<!-- OMA:END -->`까지의 관리 블록 안에는 직접 작성한 규칙을 넣지 않습니다.
...
Ubuntu 터미널 = tmux, Codex, agy, npm, bun, serena, oma 설치와 실행에 사용
```
...
Codex CLI와 agy CLI는 npm 기반 설치가 가장 단순합니다. Node.js는 `nvm`으로 설치하는 것을 권장합니다.
...
이 프로젝트의 agy hook 설정은 `bun`으로 TypeScript hook 파일을 실행합니다.
...
두 명령이 없다면 프로젝트 제공자에게 설치 방법을 확인하거나, 사용 중인 oh-my-agent/Serena 배포 방식에 맞춰 설치합니다. 일반적인 사용만 할 때는 `scripts/start-ai.sh`로 Codex/agy를 띄우는 것이 핵심이고, OMA는 agent/workflow 설정을 생성하거나 갱신할 때 더 중요합니다.
...
dos2unix scripts/*.sh .agy/hooks/*.sh .codex/hooks/*.sh
chmod +x scripts/*.sh .agy/hooks/*.sh .codex/hooks/*.sh
```
...
위 명령을 실행하면 `scripts/start-ai.sh`가 나머지 준비를 자동으로 처리합니다. 아래 내용은 사용자가 직접 입력하는 단계가 아니라, 스크립트가 내부에서 알아서 처리하는 일입니다. 사용자는 아무것도 더 입력하지 말고 Codex/agy 화면이 뜰 때까지 기다리면 됩니다.
...
| 4    | 스크립트가 오른쪽 창에서 .agy`를 실행함                                                             | 아니요                         |
| 5    | 스크립트가 두 창이 서로 메시지를 주고받을 수 있도록 Codex 창의 내부 주소를 `.codex-pane`에 자동 기록함 | 아니요                         |
| 6    | 스크립트가 agy 창의 내부 주소를 `.agy-pane`에 자동 기록함                                        | 아니요                         |
| 7    | 스크립트가 완성된 tmux 화면으로 들어감                                                                 | 아니요                         |

여기서 `.codex-pane`과 `.agy-pane`은 두 AI 창의 전화번호부 같은 파일입니다. 편지를 보낼 때 주소를 적어 두면 매번 주소를 다시 찾지 않아도 되는 것처럼, 이 파일들이 있으면 `ask-codex.sh`나 `ask-agy.sh`가 자동으로 올바른 창을 찾아 메시지를 보낼 수 있습니다. 사용자가 이 파일 내용을 외우거나 Codex/agy 프롬프트에 입력할 필요는 없습니다.
...
```bash
tmux display-message -p '#{pane_id}' > .agy-pane
```

이 파일들이 있어야 `scripts/ask-codex.sh`, `scripts/ask-agy.sh`가 상대 pane으로 메시지를 보낼 수 있습니다. 자동 실행을 사용했다면 이미 만들어져 있으므로 별도로 입력하지 않아도 됩니다.

## 7. pane 간 메시지 보내기

중요: `scripts/ask-codex.sh`와 `scripts/ask-agy.sh`에 프롬프트를 보낼 때는 반드시 프롬프트 전체를 큰따옴표(`"`)로 감쌉니다. 따옴표 없이 입력하면 쉘 특수문자, 공백, 괄호, 세미콜론, 변수 표기 때문에 프롬프트가 깨지거나 의도치 않은 명령처럼 해석될 수 있습니다.
...
agy pane으로 메시지를 보내려면:

```bash
./scripts/ask-agy.sh "이 내용을 분석만 해줘. 파일은 수정하지 마."
```

스크립트 내부 동작은 단순합니다.

1. `.codex-pane` 또는 `.agy-pane`에서 pane id를 읽습니다.
...
이 경우 agy에게 넘기지 않습니다. 구현/수정/테스트는 Codex 담당입니다.
...
Codex가 agy 위임 여부 질문
-> 사용자가 승인
-> Codex가 scripts/ask-agy.sh로 agy pane에 전달
-> agy가 분석 결과 작성
...
이 경우 agy에게 위임할지 먼저 물어보는 것이 자연스럽습니다.
...
사용자가 `yes`, `y`, `응`, `그래`, `보내줘`, `실행해`, `좋아`, `ㅇㅇ`처럼 승인하면 agy로 보냅니다.

### 9.2 agy에서 Codex로 넘기는 경우

agy는 아래 성격의 요청을 받으면 먼저 사용자에게 묻습니다.
...
예외적으로, 사용자가 이미 "agy 리뷰 결과를 바탕으로 수정까지 자동 진행"하라고 말한 경우에는 agy가 다시 묻지 않고 Codex로 바로 넘길 수 있습니다. 이때 agy는 Codex 프롬프트 첫 줄에 다음 marker를 넣습니다.

```text
AUTO_FIX_FROM_AGY_REVIEW
```

이 marker는 "agy가 먼저 검토했고, 사용자가 리뷰 결과 기반 수정 자동 위임을 허용한 요청"이라는 표시입니다. 초보자는 이 marker를 직접 입력할 필요가 거의 없습니다. agy가 Codex로 자동 위임할 때 붙이는 내부 표식이며, 화면에 보이면 "AI가 agy 검토 결과를 바탕으로 Codex에게 안전한 수정 요청을 넘기는 중이구나"라고 이해하면 됩니다.
...
최종 산출물이 리서치/기획/문서 검토면 agy
리뷰 + 수정이면 Codex
넓은 QA/방향성 리뷰만 필요하면 agy
```
...
- design
- docs
=> google.agy-3.1-pro-preview, effort medium

retrieval
=> google.agy-3.1-flash-lite
```
...
google.agy-3.1-pro-preview ->.agy-3.1-pro-preview
```
...
### 13.2 `agy pane id file not found.`
...
Terraform 작업이 아니라면 일반 Codex/agy 2분할 사용 자체에는 큰 문제가 아닐 수 있습니다.
...
코드 수정은 Codex가 담당
리서치/기획/긴 리뷰는 agy가 담당
agy 분석 결과를 무조건 적용하지 말고 Codex가 다시 검토하기
...
- Google agy CLI GitHub: https://github.com/google.agy.agy-cli
- agy CLI documentation: https://google.agy.github.io.agy-cli/
